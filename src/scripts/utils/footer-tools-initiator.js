...
const FooterToolsInitiator = {
  ...
  async _subscribePushMessage(event) {
    ...
    try {
      ...
      console.log('PUSH message has been subscribe');
    } catch (err) {
  ...
  async _isNotificationReady() {
    if (!NotificationHelper._checkAvailability()) {
      console.log('Notification not supported int this browser');
      return false;
    }
    if (!NotificationHelper._checkPermission()) {
      console.log('User did not granted  the notification permissione yet');
      const status = await Notification.requestPermission();

      if (status === 'denied') {
        // eslint-disable-next-line no-alert
        window.alert('cannot subscribe to push message because the staus');
        return false;
      }

      if (status === 'default') {
        // eslint-disable-next-line no-alert
        window.alert('Cannot subscribe to push message because the status');
        return false;
      }
    }
